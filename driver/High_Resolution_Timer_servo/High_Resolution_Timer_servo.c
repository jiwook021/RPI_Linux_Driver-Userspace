...
static struct hrtimer sample_hr_timer;
...
{
    timer_setup(&timer, servo_kernel_timer_func, 0);
    timer.expires = get_jiffies_64() + 2;
    add_timer(&timer);
}
/*
...
static int __init sample_driver_init(void)
{
    ...
    hrtimer_init(&sample_hr_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
    sample_hr_timer.function = &timer_callback;
    hrtimer_start( &sample_hr_timer, ktime, HRTIMER_MODE_REL);
 
    //led initaliztion
    gpio_request(GPIO_SERVO, "LED");
...
{
    //stop the timer
    hrtimer_cancel(&sample_hr_timer);
